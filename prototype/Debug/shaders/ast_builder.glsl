...
layout(std430,binding=0)readonly volatile buffer _screen_buffer
{
	reimp_screen _r_screen[];
};
layout(std430,binding=1)writeonly volatile buffer screen_buffer
{
...
void subtitute_tokens(in int t_pos, in int i_offset){
	int tree_row=0;
	int internal_char=0;
	...
	int final_alph=0;
	int return_pos=t_pos*4+i_offset;
	int operands[10];
	...
	if(final_val!=0){
		...
		if(codegen_data[final_val] != 0)
			volume += codegen_data[codegen_data[final_val]]+2;
...
void main(){
	...
	int t_pos=int(gl_GlobalInvocationID.x);
	w_ast_data[t_pos]=r_ast_data[t_pos];
	memoryBarrierBuffer();
	barrier();
	//glMemoryBarrier(GL_ALL_BARRIER_BITS);
	for(int i=0;i<4;i++){
		subtitute_tokens(t_pos, i);
	}
	...
	//int _pos= gl_GlobalInvocationID.x*4;
	memoryBarrierBuffer();
	barrier();
	//_r_screen[_pos].screen+=3;
