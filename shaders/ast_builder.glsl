...
layout(binding=6,offset=4)uniform atomic_uint entry_node_index;
layout(binding=6,offset=8)uniform atomic_uint entry_node_volume;
...
void subtitute_tokens(in ivec2 pos,in ivec2 dims,in int i_offset){
	...
	while((internal_char==0||tree_row!=0)&&internal_char<100){
		int pos=return_pos+internal_char;
		int current_char=_r_screen[pos]._screen;
		int current_size=_r_screen[pos]._size;
		int current_ast_pointer=_r_screen[pos]._ast_pointers;
		//deal with ignores
		bool dont_ignore;
		if(expects_anything){
	...
	if(final_val!=0){
		...
		for(int i=0;i<10;i++){
			...
			if(ind!=0){
				...
				final_operands[ind-1]=operands[i];
